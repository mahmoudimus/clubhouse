"""
Shamelessly stolen from @thieman https://github.com/thieman/py-dag

A few modifications made for this library.
"""
...
class DAG(object):
    ...
    def add_edge(self, ind_node, dep_node, graph=None):
        ...
        if ind_node not in graph:
            raise KeyError(
                'independent node "{}" does not exist in graph'
                .format(ind_node))
        if dep_node not in graph:
            raise KeyError(
                'dependent node "{}" does not exist in graph'
                .format(dep_node))
    ...
    def validate(self, graph=None):
        ...
        try:
            ...
            return (False, 'failed topological sort: {}'.format(ex))
        return (True, 'valid')
